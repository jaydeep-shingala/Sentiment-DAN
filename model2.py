...
def get_train_data():
    df = pd.read_excel("/data/home/jaydeeps/DLNLP/Assignment1/datasets/ClassificationDataset1.xlsx")
    # print(df.head())
    # df.iloc[:1] = df.iloc[:1].apply(map_label)
    print(df)
    # neg_reviews = df["Negative Review"]
    # pos_reviews = df["Positive Review"]
    # neg_reviews.append(pos_reviews)
    # df["Reviews"] = neg_reviews
    # df["Label"] = [0 for i in neg_reviews]
    # df["Reviews"] = pos_reviews
    # df["Label"] = [1 for i in pos_reviews]
    # print("***")
    # print(df.head(3))
    # Create a new DataFrame with combined "Reviews" and "Label" columns
    new_df = pd.DataFrame(columns=['Reviews', 'Label'])

    # Populate the new DataFrame based on "Negative reviews" and "Positive reviews" columns
    new_df['Reviews'] = df['Negative Review'].tolist() + df['Positive Review'].tolist()
    new_df['Label'] = [0] * len(df) + [1] * len(df)
    ...
    # X_train, X_val, y_train, y_val = train_test_split(df['Reviews'], df['Label'], test_size=0.3, random_state=42)
    ...
    # df = df.sample(frac=1, random_state=42)  # You can choose a different random_state if needed
    ...
    for index, row in train_df.iterrows():
        ...
        # print(text)
        # break
        train_data_list.append([label, text])
    
    validation_data_list = []    
    for index, row in validation_df.iterrows():
        label = row[1]
        text = row[0]
        validation_data_list.append([label, text])
...
def get_train_vectors(train_data):
    train = []
...
def get_validation_vectors(validate_data):
    validate = []
    for i, line in enumerate(validate_data):
        text = line[-1]
        # tokenizer = Tokenizer()
        # tokenizer.fit_on_texts(preprocess(text))
        # embedding_matrix_vocab = embedding_for_vocab('glove.840B.300d.txt', tokenizer.word_index, embedding_dim)
...
train = get_train_vectors(train_data)
valid = get_validation_vectors(valid_data)
...
def get_f1_score(model, data_loader):
    ...
    return f1_score(y_true, y_pred, average='weighted')  # You can change the averaging method if needed
...
mymodel = nn.Sequential(nn.Linear(300, 200),
                        ...
                        nn.ReLU(),
                        nn.Linear(30, 3))
train_network(mymodel, train_loader, valid_loader, num_epochs=30, learning_rate=1e-4)

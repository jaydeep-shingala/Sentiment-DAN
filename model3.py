...
def get_train_data():
    df = pd.read_excel("/data/home/jaydeeps/DLNLP/Assignment1/datasets/Datasets-Splits/Assignment 1/ClassificationDataset-train2.xlsx")
    # print(df.head())
    # df.iloc[:1] = df.iloc[:1].apply(map_label)
    df["ClassLabel"] = df["ClassLabel"].apply(map_label)
    data_list = []
...
def get_validate_data():
    df = pd.read_excel("/data/home/jaydeeps/DLNLP/Assignment1/datasets/Datasets-Splits/Assignment 1/ClassificationDataset-valid2.xlsx")
    # df.iloc[:1] = df.iloc[:1].apply(map_label)
    df["ClassLabel"] = df["ClassLabel"].apply(map_label)
    data_list = []
    for index, row in df.iterrows():
...
def get_train_vectors():
    ...
    for i, line in enumerate(get_train_data()):
        ...
        # vector_sum = torch.tensor([0]*embedding_dim)
        # if text:
        #     vector_sum = torch.tensor([0]*embedding_dim)
        # else:
...
mymodel = nn.Sequential(nn.Linear(300, 60000),
                        nn.Sigmoid(),
                        nn.Linear(60000, 400),
                        nn.Sigmoid(),
                        nn.Linear(400, 200),
